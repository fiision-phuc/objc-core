...
- (size_t)prepare {
    ...
    if (!allHeaders[@"Accept-Charset"]) {
        [self setValue:@"UTF-8" forHTTPHeaderField:@"Accept-Charset"];
    }
    if (!allHeaders[@"Connection"]) {
        [self setValue:@"close" forHTTPHeaderField:@"Connection"];
    }
    if (!allHeaders[@"User-Agent"]) {
        [self setValue:FwiGenerateUserAgent() forHTTPHeaderField:@"User-Agent"];
    }
    
    /* Condition validation */
    if (!_raw && _form.count == 0 && _upload.count == 0) {
        return 0;
    }
    size_t length = 0;
...
- (void)_addFormParam:(FwiFormParam *)param {
    /* Condition validation */
    if (![param isMemberOfClass:[FwiFormParam class]] || [_form containsObject:param]) {
        return;
    }
...
- (void)_addMultipartParam:(FwiMultipartParam *)param {
    /* Condition validation */
    if (![param isMemberOfClass:[FwiMultipartParam class]] || [_upload containsObject:param]) {
        return;
    }
...
- (void)setDataParameter:(FwiDataParam *)parameter {
    ...
    if (!(_type == kPost || _type == kPatch || _type == kPut) || !parameter) {
        return;
    }
